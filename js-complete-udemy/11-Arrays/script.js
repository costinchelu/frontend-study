...
// SLICE (original array remains the same)
...
// SPLICE (like slice but the original array is modified - extracted elements are gone)
...
console.log(letters);
//same result using spread operator
console.log([...arr, ...arr2]);

// JOIN (creates a string using a specified separator)
...
// get an element - we can use the index selection or a method like "at"
console.log(arr[0]);
...
// at is also good for method chaining
...
// you cannot break a foreach loop
console.log('---- FOREACH ----');
...
// Set (same function signature, but we can throw away the index/key argument
...
// map will create a new array with an applied callback function on it
...
// no arrow:
  ...
  //   return mov * eurToUsd;
  // });
// but because we have a single line function, we are better using an arrow function:
const movementsUSD = movements.map(mov => Math.trunc(mov * eurToUsd));

console.log(movements);
console.log(movementsUSD);

// without map, we would need to create the array, loop, and add to that array the modified value
const movementsUSDfor = [];
for (const mov of movements) {
  movementsUSDfor.push(Math.trunc(mov * eurToUsd));
}

// we can get access to the whole array and index through map
...
const foundAccount = accounts.find(acc => acc.owner === 'Jessica Davis');
